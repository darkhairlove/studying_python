...
#     li.append((a, b))
#     arr[a] = b
# sorted_dict = sorted(li, key=lambda item: item[1], reverse=True)
...
# 다른 사람 풀이  https://ji-gwang.tistory.com/316
...
for i in range(6):
    ...
    if temp[0] == 1 or temp[0] == 2:  # 만약 가로 길이라면
        if max_width < temp[1]:  # 해당 길이가 가장 길다면
            max_width = temp[1]  # 그 길이를 가장 긴 가로로 저장한 후
            max_width_index = i  # 그에 해당하는 인덱스를 저장
    else:
        if max_height < temp[1]:  # 해당 길이가 가장 길다면
            max_height = temp[1]  # 그 길이를 가장 긴 세로로 저장한 후
            max_height_index = i  # 그에 해당하는 인덱스를 저장

# 그 후 가장 긴 가로 및 세로와 인접한 변 2개와 가장 긴 가로와 세로 총 4개를 새로운 리스트에 저장한다.
